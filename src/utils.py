...
def ljust_combined(string, width, fillchar=" "):
    char_count = 0
    for char in string:
        char_count += wcwidth.wcwidth(char)

    padding = max(0, width - char_count)
    return string.ljust(char_count + padding, fillchar)


def rjust_combined(string, width, fillchar=" "):
    char_count = 0
    for char in string:
        char_count += wcwidth.wcwidth(char)

    padding = max(0, width - char_count)
    return string.rjust(char_count + padding, fillchar)
...
def update_github_repo_secret(repo, github_token, secret_records):
    ...
    public_key = public.PublicKey(
        public_key_info["key"].encode("utf-8"), encoding.Base64Encoder()
    )
